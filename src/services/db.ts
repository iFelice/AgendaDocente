...
import type { CalendarEvent, CircularDocument, Student, StudentAssessment, TeacherProfile, TimetableSlot, TimetableMode, TimeSlotConfig } from '../types';
...
export const LEGACY_KEYS = {
  ...
  timeSlotConfig: 'agedoc_time_slot_config_v2', assessments: 'agedoc_assessments_v1',
} as const;
export interface LocalData {
  profile: TeacherProfile; events: CalendarEvent[]; circulars: CircularDocument[]; students: Student[]; assessments: StudentAssessment[];
...
const collections = ['events','circulars','students','assessments','definitiveTimetable','provisionalTimetable'] as const;
...
export class AgendaDatabase extends Dexie {
  ...
  constructor(name = 'agenda-docente') {
    ...
    const legacySchema = Object.fromEntries(stores.filter(name => name !== 'assessments').map(name => [name, name === 'metadata' ? '&key' : '&id,position']));
  ...
  private ordered(data: LocalData): LocalData {
    return {profile:data.profile,events:data.events,circulars:data.circulars,students:data.students,assessments:data.assessments ?? [],
