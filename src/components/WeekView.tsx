      ...
      <div className="bg-white rounded-xl p-4 border border-stone-200 shadow-xs flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2 flex-wrap min-w-0">
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-lg border border-stone-200 hover:bg-stone-50 text-xs font-semibold text-stone-700 transition-colors"
          >
          ...
          {isWeekendToday && currentWeekOffset === 0 && (
            <span className="hidden sm:inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-semibold bg-emerald-50 text-emerald-800 border border-emerald-200 ml-2">
              Lunedì prossimo in arrivo
            </span>
          )}
          {isProvisionalTimetable && (
            <span
              className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold bg-amber-50 text-amber-900 border border-amber-300 ml-2"
              title="Orario provvisorio per i primi giorni di scuola attivo"
            >
              <Clock className="w-3.5 h-3.5 mr-1 text-amber-700" />
              <span>Orario Provvisorio</span>
            </span>
          )}
        </div>
                              ...
                              <div className="flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                                {confirmingDeleteEventId === ev.id ? (
                                  ...
                                  <button
                                    ...
                                    className="opacity-40 group-hover:opacity-100 p-0.5 text-stone-400 hover:text-rose-600 hover:bg-rose-50 rounded transition-all"
