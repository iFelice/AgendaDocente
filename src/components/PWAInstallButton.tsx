  ...
  if (isInstallable) {
    return (
      <>
        ...
        {installSuccess && (
          <div className="fixed bottom-5 left-5 z-50 bg-emerald-800 text-white px-4 py-2 rounded-lg text-xs flex items-center space-x-2 shadow-lg">
        ...
        {showIOSGuide && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-900/50 backdrop-blur-xs p-4">
            <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl border border-stone-200 text-xs">
              ...
              <button
                onClick={() => setShowIOSGuide(false)}
                className="mt-5 w-full rounded-xl bg-emerald-700 py-2.5 text-xs font-bold text-white hover:bg-emerald-800 transition-colors shadow-xs"
              >
                Ho Capito
              </button>
            </div>
